...
15. **Mandatory GitHub Actions & SonarCloud CI Waiting Strategy:** Whenever checking or waiting for GitHub Actions workflow runs or SonarCloud analysis results (including executing commands starting with `gh run view` after a push or workflow trigger), the agent MUST ALWAYS assume an initial wait time of 2 minutes (120 seconds) before checking status for the first time, and MUST recheck status only after waiting at least 45 seconds between subsequent status polls.
16. **Mandatory Builder Pattern for Complex Records:** All Java `record` classes or domain value objects with 4 or more fields (e.g. `UploadSession`) MUST provide a fluent builder (`@Builder(toBuilder = true)`). Instantiations across production and test code MUST use `Record.builder()...build()` or `record.toBuilder()`.
